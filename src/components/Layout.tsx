...
const Layout = memo(({ currentPage, setCurrentPage, children }: LayoutProps) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col bg-gray-900 text-white ${isMenuOpen ? 'overflow-hidden' : ''}`}>
      <div className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ease-in-out ${
        isScrolled ? 'h-16' : 'h-20'
      } ${isMenuOpen ? 'bg-transparent' : isScrolled ? 'bg-gray-900/70 backdrop-blur-md' : 'bg-gray-900'}`}>
        <div className="container mx-auto px-4 h-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <nav className="hidden md:block">
              <div className="relative flex gap-2 p-1 bg-gray-700/80 rounded-lg">
                <div 
